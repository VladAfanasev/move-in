...
import { generateCalculationQRInviteAction } from "@/actions/groups/qr-invite-calculation"
import { generateShareLinkAction } from "@/actions/groups/share"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover"

interface CalculationInvitePopoverProps {
...
export function CalculationInvitePopover({
  children,
  groupId,
  propertyId,
  groupName,
}: CalculationInvitePopoverProps) {
  ...
  const generateQRInviteUrl = useCallback(async () => {
    ...
    setQrLoading(true)
    try {
      const result = await generateCalculationQRInviteAction(groupId, propertyId)
      if (result.success) {
      ...
      console.error("Error generating calculation QR invite URL:", error)
    } finally {
      setQrLoading(false)
    }
  }, [groupId, propertyId, qrInviteUrl, qrLoading])
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-96">
        <div className="space-y-6">
          <div>
            <h4 className="font-medium">Leden uitnodigen voor berekening</h4>
            <p className="text-muted-foreground text-sm">
              Deel de QR-code of link om nieuwe leden direct naar deze berekening te brengen
            </p>
          </div>
                ...
                <>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        const message = `Hallo! Je bent uitgenodigd om lid te worden van de koopgroep "${groupName}" en deel te nemen aan de kostenberekening. Klik op deze link: ${qrInviteUrl}`
