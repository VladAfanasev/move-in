...
import { AlertCircle, CheckCircle, Scale, Target, User as UserIcon } from "lucide-react"
...
export function LiveNegotiationSession({
  ...
  const getProgressMessage = () => {
    ...
    if (totalPercentage === 100 && allConfirmed) {
      return "🎉 ALL MEMBERS CONFIRMED"
    }
    if (totalPercentage === 100) {
      return "100% reached! Waiting for everyone to confirm..."
    }
    if (totalPercentage > 100) {
      return `Over limit! ${(totalPercentage - 100).toFixed(1)}% too much`
    }
    return ""
  }
